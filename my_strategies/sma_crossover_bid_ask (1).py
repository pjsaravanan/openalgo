...
EXCHANGE_INDEX = {'NIFTY':'NFO', 'SENSEX':'BFO'}
INTERVAL = '1m'
MAX_DAYS = 7
...
def discover_expiries(symbol_base, exchange):
    ...
    try:
        if hasattr(client, 'get_expiries'):
            raw = client.get_expiries(symbol=symbol_base, exchange=exchange)
            expiries = [pd.to_datetime(x).date() for x in raw]
...
def run_backtests(params):
    ...
    for underlying in INDEX_UNDERLYINGS:
        exchange = EXCHANGE_INDEX.get(underlying, 'NFO')
